...
if __name__ == "__main__":
	...
	for dataset in fitResult.datasetLabels():
		# get normalization integral
		psIntegralPath  = fileManager.getIntegralFilePath(multiBin, pyRootPwa.core.eventMetadata.GENERATED, dataset)
		...
		eventAndAmpFileDict = fileManager.getEventAndAmplitudeFilePathsInBin(multiBin, eventType, dataset)
		...
		for dataFilename in sorted(eventAndAmpFileDict.keys()):
			...
			datasetRatio = fitResult.datasetRatio(dataset)
			...
			for iEvent in xrange(nEvents):
				...
				if multiBin.inBin(binningVariables): # @todoTODO: use c++ function when merged
					...
					weight[0] = datasetRatio * pyRootPwa.core.spinDensityMatrixTimesAmplitudeMatrix(waveIndices, spinDensityMatrix, decayAmplitudeMatrix)
